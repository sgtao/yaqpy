"""argparse definition for the ``pyyq`` command (design doc 12-2)."""
...
from pyyq.formats.registry import builtin_formats
...
def build_parser() -> _Parser:
    ...
    parser = _Parser(
        prog="pyyq",
        description="pyyq is a lightweight and portable command-line data file processor "
                    "(a pure-Python implementation of mikefarah/yq).",
        epilog="""examples:
  pyyq '.stuff' < myfile.yml
  pyyq -i '.stuff = "foo"' myfile.yml
  pyyq -P -oy sample.json
  pyyq eval-all 'select(fi == 0) * select(fi == 1)' f1.yml f2.yml""",
