...
- **Web 版の GUI も同じ画面**：`yaqpy-gui --web` は、デスクトップ版と同じ Flet の画面を `flet.fastapi` ＋ uvicorn で配ります（`gui/_web.py`）。評価は `SandboxFileSystem` と空の環境変数の上で動き、サーバーのファイル・環境変数に届きません（`gui/_di.py` の `make_web_service`）。Flet の Web 表示の実測は `docs/flet-1.0-api-notes.md` の 7 章
...
# ユニットテスト（1,268 件。各形式・schema・演算子（性質テストを含む）・レシピ・自己説明・入力形式の自動判定・GUI の Presenter など。実際にウィンドウは開きません。Web 版のサーバーは、[web] extra があれば 127.0.0.1 の空きポートで実際に起動して確かめます）
uv run pytest tests/unit -n auto

# CLI 受け入れテスト（84 件。Go 版 acceptance_tests/*.sh から移植（`-s` の分割出力を含む）＋`--gui` の入口＋レシピ・自己説明・自動判定（実プロセスでの stdout/stderr の分離など））
...
└── gui/                            … Flet の GUI（任意依存。presenter は Flet 非依存）。_run（画面の組み立て。デスクトップ・Web 共通）、_web（Web 版のサーバー。uvicorn を使うのはここだけ）、_upload（Web 版のアップロード受け取り）、web_config（Web 版の設定・上限。Flet 非依存）
tests/
