...
**GUI をブラウザで使える版です。** `yaqpy-gui --web` で、デスクトップ版と同じ画面を**自分の PC の小さな Web サーバー**から配り、ブラウザで開けます。ファイルはブラウザから**アップロード**し、変換結果は**ダウンロード**で受け取ります。既定では**この PC からしか開けず**（`127.0.0.1`）、環境変数・ファイルを読む演算子は**常に無効**で、評価はサーバーのファイル・環境変数に届かない仕組みの上で動きます。入れ方は `yaqpy[web]`、使い方は [USAGE-GUI.ja.md の 15 章](USAGE-GUI.ja.md#15-web-版ブラウザで使う)にあります。

### 新しくできること

| 分類 | 機能 |
|---|---|
| **Web 版の起動** | `yaqpy-gui --web`（既定は `http://127.0.0.1:8550/`、起動したら既定のブラウザを開く）。`--host` `--port` `--lang ja\|en`（表示言語。すべてのブラウザで共通）`--no-browser` `--no-cdn` `--max-input-mib`（既定 10）`--timeout`（既定 10 秒）`--max-concurrent-runs`（既定 2）。止めるのは起動した端末で `Ctrl + C` |
...
- **`yaqpy-gui` が引数を解釈するようになりました。** 以前は何を渡しても無視して窓を開いていました。今は、知らないオプション・存在しないファイル・`--web` なしでの Web 用オプションは、エラー（終了コード 1）になります。`yaqpy --gui` の挙動は変わりません
- **クリップボードへのコピーが失敗したとき**（ブラウザや OS が許可しないとき）、画面が例外で止まらず「コピーできませんでした。欄の文字を選択してコピーしてください」と案内するようになりました（デスクトップ版も同じ）

### ライブラリ・開発者向けの変更

- `pyproject.toml` の `yaqpy-gui` の行き先を `yaqpy.gui.app:main_entry` から **`yaqpy.gui.app:cli_entry`** に変えました（引数の解釈のため）。`main_entry` は今までどおり残り、`yaqpy --gui` から呼ばれます
...
- 単体テスト 1,168 → **1,268**（Web 版のサーバーを実際に起動して、`127.0.0.1` だけで待ち受けること・署名の無いアップロードを断ることを確かめるテストを含む。`[web]` extra が無い環境では飛ばします）。CLI 受け入れテストは 84 件のまま
