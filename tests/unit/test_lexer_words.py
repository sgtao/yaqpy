    ...
    def test_each_spelling_is_read_as_its_own_rule_and_in_full(self) -> None:
        ...
        for rule in DEFAULT_RULES:
            for word in _spellings(rule.pattern):
                ...
                assert (matched_rule.name, match.end()) == (rule.name, len(word))
        assert checked > 100      # the table really was walked

    def test_as_and_ref_still_bind_variables_however_they_are_spaced(self) -> None:
        import yaqpy

        assert yaqpy.query(". as $x | $x.a", {"a": 1}) == [1]
        assert yaqpy.query(".a as$x|$x", {"a": 2}) == [2]
        assert yaqpy.query(".a as $x | .b as $y | $x + $y", {"a": 1, "b": 2}) == [3]

    def test_the_ascii_case_operators_work(self) -> None:
        import yaqpy

        assert yaqpy.query("ascii_downcase", "ABC") == ["abc"]
        assert yaqpy.query("ascii_upcase", "abc") == ["ABC"]
