...
class EncodeTests:
    def test_json_indent(self) -> None:
        assert run(".a | to_json", "a: {b: 1}") == '{\n  "b": 1\n}\n'
        assert run(".a | to_json(0)", "a: {b: 1}") == '{"b":1}'
        assert run(".a | @json", "a: {b: 1}") == '{"b":1}'
        assert run(".a | to_json(4)", "a: {b: 1}") == '{\n    "b": 1\n}\n'

    def test_a_scalar_is_encoded_with_its_quotes_in_json(self) -> None:
        assert run(".a | @json", "a: hi") == '"hi"'

    def test_yaml_and_props(self) -> None:
        assert run(".a | to_yaml", "a:\n  b: 1") == "b: 1\n"
        assert run(".a | @props", "a:\n  b: 1") == "b = 1\n"

    def test_csv_and_tsv_have_no_trailing_newline(self) -> None:
        assert run("@csv", "[[a, b], [c, d]]") == "a,b\nc,d"
        assert run("@tsv", "[[a, b], [c, d]]") == "a\tb\nc\td"

    def test_xml(self) -> None:
        assert run(".a | @xml", "a: {x: 1}").strip() == "<x>1</x>"

    def test_aliases_are_expanded_for_json_without_touching_the_input(self) -> None:
        text = "a: &x {b: 1}\nc: *x"
        assert run(".c | @json", text) == '{"b":1}'
        # encoding must not expand the alias in the document itself
        assert yaqpy.evaluate("(.c | @json) as $j | .", text) == text + "\n"
...
class RoundTripTests:
    ...
    def test_decoding_an_empty_string_gives_null(self) -> None:
        assert run('"" | from_yaml') is None
        assert run('"" | @csvd') is None

    def test_decoding_keeps_the_path(self) -> None:
        assert run(".a | from_json | path | .[0]", 'a: \'{"x": 1}\'') == "a"
...
class Base64Tests:
    def test_encode_and_decode(self) -> None:
        assert run("@base64", "a special string") == "YSBzcGVjaWFsIHN0cmluZw=="
        assert run("@base64d", "YSBzcGVjaWFsIHN0cmluZw==") == "a special string"

    def test_missing_padding_is_added(self) -> None:
        assert decode_base64("Y2F0cw") == "cats"

    def test_surrounding_whitespace_and_line_breaks_are_ignored(self) -> None:
        assert decode_base64("  Y2F0\ncw==\n") == "cats"

    def test_utf8(self) -> None:
        assert run("@base64 | @base64d", "日本語") == "日本語"
    ...
    def test_illegal_data(self) -> None:
        with pytest.raises(EvaluationError) as raised:
            decode_base64("ab!d")
        assert str(raised.value) == "illegal base64 data at input byte 2"

    def test_only_strings_are_encoded(self) -> None:
        assert fails("@base64", "3") == "cannot encode !!int as base64, can only operate on strings"


class UriTests:
    def test_encode(self) -> None:
        assert run("@uri", "this has & special () characters *") == "this+has+%26+special+%28%29+characters+%2A"
        assert run("@uri", "a-b_c.d~e") == "a-b_c.d~e"
        assert run("@uri", "日") == "%E6%97%A5"

    def test_decode(self) -> None:
        assert decode_uri("a+b%20c%E6%97%A5") == "a b c日"

    def test_bad_escapes(self) -> None:
        for text, shown in (("%zz", "%zz"), ("100%", "%"), ("a%4", "%4")):
            with pytest.raises(EvaluationError) as raised:
                decode_uri(text)
            assert str(raised.value) == f'invalid URL escape "{shown}"'

    def test_only_strings_are_encoded(self) -> None:
        assert "cannot encode !!int as URI" in fails("@uri", "3")


class ShTests:
    def test_safe_text_is_left_alone(self) -> None:
        assert run("@sh", "abc/def-1.2_3@x%y+z=w:v,u") == "abc/def-1.2_3@x%y+z=w:v,u"

    def test_unsafe_text_is_quoted(self) -> None:
        assert run("@sh", "a b") == "a' b'"
        assert run("@sh", "it's") == "it\\'s"

    def test_quotes_and_spaces_mix(self) -> None:
        assert run("@sh", "strings with spaces and a 'quote'") == "strings' with spaces and a '\\'quote\\'"

    def test_only_strings(self) -> None:
        assert "cannot encode !!bool as URI" in fails("@sh", "true")
