...
class ObjectTests:
    ...
    def test_array_elements_are_merged_recursively(self) -> None:
        ...
        assert node["properties"] == {"x": {"type": "integer"}, "y": {"type": "string"}}
        assert node["required"] == ["x"]

    def test_strict_closes_every_object(self) -> None:
        schema = schema_of("a: {b: 1}\nc: [{d: 1}]\n", strict=True)
        assert schema["additionalProperties"] is False
        assert schema["properties"]["a"]["additionalProperties"] is False
        assert schema["properties"]["c"]["items"]["additionalProperties"] is False
        assert "additionalProperties" not in schema_of("a: {b: 1}\n")

    def test_merge_keys_are_expanded_on_a_copy(self) -> None:
        text = "base: &b {x: 1}\nchild:\n  <<: *b\n  y: 2\n"
        schema = schema_of(text)
        assert list(schema["properties"]["child"]["properties"]) == ["x", "y"]
...
class StringTests:
    def test_timestamps_get_a_format_when_all_agree(self) -> None:
        ...
        assert schema["properties"]["a"] == {"type": "string", "format": "date-time"}
        assert schema["properties"]["b"] == {"type": "string", "format": "date"}
        assert schema["properties"]["c"] == {"type": "string", "format": "date-time"}
    ...
    def test_enum_is_off_by_default(self) -> None:
        assert "enum" not in schema_of("- a\n- b\n- a\n")["items"]

    def test_enum_for_repeating_values(self) -> None:
        schema = schema_of("- a\n- b\n- a\n", enum_max=5)
        assert schema["items"] == {"type": "string", "enum": ["a", "b"]}
...
class MultipleNodesTests:
    ...
    def test_it_can_sit_in_a_pipe(self) -> None:
        ...
        assert list(schema["properties"]) == ["n", "m"]
        assert schema_of(text, expression=".items | schema")["type"] == "array"

    def test_the_result_can_be_processed_further(self) -> None:
        options = Options(output_format="yaml", indent=0)
        assert yaqpy.evaluate("schema | .properties | keys | .[]", "a: 1\nb: 2\n",
                              options=options) == "a\nb\n"

    def test_nothing_in_nothing_out(self) -> None:
        options = Options(output_format="json")
        assert yaqpy.evaluate(".nothing[] | schema", "a: 1\n", options=options) == ""

    def test_schema_is_a_key_name_too(self) -> None:
        text = "schema: 1\nx:\n  schema: y\n"
        options = Options(output_format="yaml")
        assert yaqpy.evaluate(".x.schema", text, options=options) == "y\n"
        assert yaqpy.evaluate("{\"schema\": .schema}", text, options=options) == "schema: 1\n"
...
class OutputTests:
    def test_json_and_yaml_say_the_same(self) -> None:
        ...
        assert json.loads(yaqpy.evaluate(".", as_yaml, options=Options(output_format="json"))) == as_json
...
class LimitTests:
    ...
    def test_step_limit_applies(self) -> None:
        ...
        with pytest.raises(yaqpy.EvaluationError):
...
class ValidityTests:
    ...
    def test_every_test_set_gets_a_schema_that_accepts_it(self) -> None:
        ...
        for name, docs in self.samples():
            ...
            if merged is None:
                continue
            assert check_structure(merged) == []
            for doc in docs:
                assert validate(json.loads(doc), merged) == [], f"{name}: {doc[:80]}"
            checked += 1
        assert checked >= 9
    ...
    def test_each_test_set_file_gives_a_schema_from_the_cli(self) -> None:
        ...
        for path in files:
            ...
            for line in out.splitlines():
                assert json.loads(line)["$schema"] == DIALECT
    ...
    def test_random_data_validates_against_its_own_schema(self) -> None:
        ...
        for round_ in range(40):
            ...
            for strict in (False, True):
                ...
                assert check_structure(merged) == [], docs
                for doc in docs:
                    assert validate(json.loads(doc), merged) == [], f"{docs} / {merged}"


class CliTests:
    @pytest.fixture(autouse=True)
    def _directory(self, tmp_path) -> None:
    ...
    def test_the_flag_can_follow_an_expression(self) -> None:
        ...
        assert json.loads(out)["required"] == ["b"]
    ...
    def test_option_flags(self) -> None:
        ...
        assert schema["items"]["properties"]["k"]["enum"] == ["a", "b"]
