...
class ListRecipesTests(unittest.TestCase):
    def test_lists_the_bundled_recipes_by_name(self) -> None:
        ...
        self.assertIn("openai-to-gemini", recipes)
        self.assertEqual(recipes["openai-to-gemini"].input_api, "openai-chat-completions")

    def test_the_returned_dictionary_is_a_copy(self) -> None:
        yaqpy.list_recipes().clear()
        self.assertTrue(yaqpy.list_recipes())
...
class ApplyRecipeTests(unittest.TestCase):
    ...
    def test_converts_text_and_reports(self) -> None:
        ...
        self.assertEqual(json.loads(run.output), {"contents": [{"role": "user", "parts": [{"text": "Hello"}]}]})
        self.assertEqual((run.input_format, run.output_format, run.document_count), ("json", "json", 1))
        self.assertEqual(sorted(d.path for d in run.report.dropped), [".model", ".stream"])
        self.assertTrue(run.report.clean)
    ...
    def test_a_recipe_object_built_by_the_caller(self) -> None:
        ...
        self.assertEqual(json.loads(run.output), {"n": 1})
        self.assertEqual([d.path for d in run.report.not_handled], [".b"])

    def test_output_format_can_be_changed(self) -> None:
        run = yaqpy.apply_recipe("openai-to-gemini", self.BODY, output_format="yaml")
        self.assertTrue(run.output.startswith("contents:\n"))

    def test_input_may_be_yaml(self) -> None:
        run = yaqpy.apply_recipe("openai-to-gemini", "model: m\nmessages:\n  - {role: user, content: hi}\n",
                                 input_format="yaml")
        self.assertEqual(json.loads(run.output)["contents"][0]["parts"], [{"text": "hi"}])

    def test_prune_options(self) -> None:
        recipe = yaqpy.build_recipe(name="p", expression='{"a": null, "b": {}, "c": 1}', metadata=None,
                                    origin="p")
        self.assertEqual(json.loads(yaqpy.apply_recipe(recipe, "{}", prune_null=True).output),
                         {"b": {}, "c": 1})
        self.assertEqual(json.loads(yaqpy.apply_recipe(recipe, "{}", prune_null=True, prune_empty=True).output),
                         {"c": 1})
    ...
    def test_a_recipe_never_reads_the_environment_even_if_the_options_allow_it(self) -> None:
        os.environ["YAQPY_API_SECRET"] = "hunter2"
        self.addCleanup(os.environ.pop, "YAQPY_API_SECRET", None)
        ...
        with self.assertRaises(yaqpy.SecurityError):
