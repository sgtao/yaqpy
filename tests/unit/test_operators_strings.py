...
class RegexOperatorTests:
    ...
    def test_a_comma_is_not_a_params_separator(self) -> None:
        ...
        assert len(run('[match("a", "g")]', "banana")) == 1

    def test_capture(self) -> None:
        assert run('capture("(?P<a>[a-z]+)-(?P<n>[0-9]+)")', "cat-42") == {"a": "cat", "n": "42"}
        assert run('capture("(?P<a>x)?b")', "b") == {"a": None}

    def test_test_anchors_at_the_end_of_the_text(self) -> None:
        assert run('test("cat$")', "cat") is True
        # a YAML block scalar ends with a newline; Go's $ does not match before it
        assert run('test("cat$")', "|\n  cat\n") is False

    def test_test_refuses_non_strings(self) -> None:
        assert "cannot match with !!int" in fails('test("1")', "1")
...
class TrimCaseTests:
    ...
    def test_trim_uses_go_whitespace(self) -> None:
        assert run("trim", '"\u3000x\u00a0"') == "x"
        assert run("trim", '"\\u001cx"') == "\x1cx"     # not whitespace for Go

    def test_trim_refuses_non_strings(self) -> None:
        assert fails("trim", "3") == "cannot trim !!int, can only operate on strings. "

    def test_case(self) -> None:
        assert run("upcase", "abc Def") == "ABC DEF"
        assert run("downcase", "ABC Def") == "abc def"

    def test_case_maps_each_character_on_its_own(self) -> None:
        assert run("upcase", "straße") == "STRAßE"     # Python's str.upper gives STRASSE

    def test_case_refuses_non_strings(self) -> None:
        assert fails("upcase", "3") == "cannot change case with !!int, can only operate on strings. "


class ToStringNumberTests:
    def test_to_string(self) -> None:
        assert run("to_string", "12") == "12"
        assert run("to_string", "true") == "true"
        assert run("to_string", "a") == "a"
        assert run("to_string", "b: 1\nc: 2") == "b: 1\nc: 2"

    def test_to_string_quotes_non_strings_in_yaml(self) -> None:
        assert yaqpy.evaluate("to_string", "12", options=Options(unwrap_scalar=False)) == '"12"\n'

    def test_to_number(self) -> None:
        assert run("[.[] | to_number]", '["3", "3.5", "-1e3", "0x1F", "+7", "1_000"]') == [3, 3.5, -1000.0, 31, 7, 1000]

    def test_to_number_tags(self) -> None:
        out = yaqpy.evaluate(".[] | to_number | tag", '["3", "3.5", "1e3"]')
        assert out.split() == ["!!int", "!!float", "!!float"]

    def test_int64_overflow_becomes_a_float(self) -> None:
        assert yaqpy.evaluate("to_number | tag", '"99999999999999999999"').strip() == "!!float"

    def test_special_floats(self) -> None:
        assert yaqpy.evaluate("to_number | tag", '"NaN"').strip() == "!!float"
        assert yaqpy.evaluate("to_number | tag", '"-Inf"').strip() == "!!float"
        assert "cannot convert node value [+nan]" in fails("to_number", '"+nan"')

    @pytest.mark.parametrize("text", ('"abc"', '" 1"', '"1 "', '"1_0.5"', '"0x"', '""'))
    def test_not_numbers(self, text) -> None:
        assert "cannot convert node value" in fails("to_number", text)

    def test_containers_are_refused(self) -> None:
        assert "cannot convert node at path" in fails("to_number", "[1]")


class InterpolationTests:
    def test_values(self) -> None:
        assert run('"I like \\(.a) and \\(.b)"', "{a: cats, b: dogs}") == "I like cats and dogs"

    def test_expressions_and_nested_parentheses(self) -> None:
        assert run('"n=\\(.a | length)"', "a: [1, 2, 3]") == "n=3"
        assert run('"n=\\((.a | length) + 1)"', "a: [1, 2, 3]") == "n=4"
        assert run('"\\( (.a) )"', "a: hi") == "hi"

    def test_an_escaped_backslash_is_kept(self) -> None:
        assert run('"Hi \\\\(.a)"', "a: x") == "Hi \\(.a)"

    def test_a_map_is_written_as_yaml(self) -> None:
        assert run('"got \\(.a)"', "a:\n  b: 1") == "got b: 1"

    def test_an_empty_result_is_an_empty_string(self) -> None:
        assert run('"[\\(.zzz | select(. != null))]"', "a: 1") == "[]"

    def test_unclosed_is_left_alone(self) -> None:
        assert run('"a \\(.a"', "a: 1") == "a \\(.a"

    def test_it_can_be_turned_off(self) -> None:
        assert run('"a \\(.a)"', "a: 1", string_interpolation=False) == "a \\(.a)"
    ...
    def test_cli_flag(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for flag, want in (("--string-interpolation=false", "a \\(.a)\n"),
                ...
                assert (code, out.getvalue()) == (0, want)
