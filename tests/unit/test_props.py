...
class ParseTests:
    ...
    def test_separators(self) -> None:
        text = "a=1\nb = 2\nc: 3\nd : 4\ne 5\nf\t6\n"
        assert self.pairs(text) == [("a", "1"), ("b", "2"), ("c", "3"), ("d", "4"),
                                  ("e", "5"), ("f", "6")]

    def test_value_keeps_everything_after_the_separator(self) -> None:
        assert self.pairs("a =  x = y:z  \n") == [("a", "x = y:z  ")]
    ...
    def test_line_continuation(self) -> None:
        text = "a = one \\\n    two \\\n  three\nb = 2\nc = end\\\n"
        assert self.pairs(text) == [("a", "one two three"), ("b", "2"), ("c", "end")]

    def test_an_escaped_backslash_does_not_continue(self) -> None:
        assert self.pairs("a = x\\\\\nb = y\n") == [("a", "x\\"), ("b", "y")]

    def test_escapes_in_values_and_keys(self) -> None:
        text = "a = tab\\there\\nnew \\u00e9 \\q\nk\\ ey\\:x = 1\n"
        assert self.pairs(text) == [("a", "tab\there\nnew é q"), ("k ey:x", "1")]
    ...
    def test_later_definition_replaces_the_value_but_keeps_the_position(self) -> None:
        assert self.pairs("a=1\nb=2\na=3\n") == [("a", "3"), ("b", "2")]

    def test_comments_belong_to_the_next_property(self) -> None:
        entries = parse_properties("# one\n! two\n\na = 1\n# lonely\n")
        assert entries == [("a", "1", ["one", "two"])]

    def test_key_paths(self) -> None:
        assert parse_key("a.b.0.c", array_brackets=False) == ["a", "b", 0, "c"]
        assert parse_key("a.b[0][1].c", array_brackets=True) == ["a", "b", 0, 1, "c"]
        assert parse_key("a.b[0]", array_brackets=False) == ["a", "b[0]"]
        assert parse_key("a.b[x]", array_brackets=True) == ["a", "b[x]"]
        assert parse_key("a.-1.+2", array_brackets=False) == ["a", "-1", 2]


class DecodeTests:
    def test_nested_maps_and_lists(self) -> None:
        text = "a.b = 1\na.c = 2\nd.0 = x\nd.1.k = y\n"
        assert read(text) == {"a": {"b": "1", "c": "2"}, "d": ["x", {"k": "y"}]}
    ...
    def test_limits(self) -> None:
        ...
        assert len(read(f"a.{MAX_ARRAY_INDEX} = x\n")["a"]) == MAX_ARRAY_INDEX + 1
        with pytest.raises(FormatError):
    ...
    def test_error_names_the_file(self) -> None:
        ...
        assert str(ctx.value).startswith("bad file 'x.properties': properties: cannot set")
...
class WriteTests:
    ...
    def test_special_characters_survive_a_round_trip(self) -> None:
        ...
        assert json.loads(yaqpy.evaluate(".", text, options=options)) == {"k ey:1": "tab\there", "line": "a\nb", "path": "C:\\temp", "uni": "é日本"}
...
class CliTests:
    def test_extension_and_input_formats(self) -> None:
        ...
        assert formats.from_filename("app.properties").name == "props"
        assert "props" in formats.input_formats()
        assert "properties" in formats.input_extensions()
