...
class DatetimeProperties:
    ...
    def test_parse_of_format_is_the_same_instant(self) -> None:
        ...
        for _ in range(400):
            ...
            for layout in self.LAYOUTS:
                ...
                assert parsed.utcoffset() == moment.utcoffset()
    ...
    def test_a_two_digit_year_round_trips_from_1969_to_2068(self) -> None:
        ...
        for _ in range(300):
            ...
            assert parse_datetime(layout, format_datetime(moment, layout)) == moment

    def test_numeric_fields_agree_with_strftime(self) -> None:
        rng = random.Random(7)
        for _ in range(500):
            moment = random_moment(rng)
            assert format_datetime(moment, "2006-01-02 15:04:05") == moment.strftime("%Y-%m-%d %H:%M:%S")
            assert format_datetime(moment, "Mon Jan 02 06 03:04PM") == moment.strftime("%a %b %d %y %I:%M%p")
            assert format_datetime(moment, "Monday, January 02") == moment.strftime("%A, %B %d")
            assert format_datetime(moment, "002") == moment.strftime("%j")
    ...
    def test_unix_seconds_agree_with_timestamp(self) -> None:
        ...
        for _ in range(300):
            ...
            assert unix_seconds(moment) == int(moment.timestamp() // 1)
...
class EncodingProperties:
    ...
    def test_base64_matches_python_and_round_trips(self) -> None:
        texts = self.texts(1)
        assert run(".[] | @base64", texts) == [base64.b64encode(t.encode()).decode() for t in texts]
        assert run(".[] | @base64 | @base64d", texts) == texts

    def test_uri_matches_python_and_round_trips(self) -> None:
        texts = self.texts(2)
        assert run(".[] | @uri", texts) == [urllib.parse.quote_plus(t, safe="") for t in texts]
        assert run(".[] | @uri | @urid", texts) == texts
    ...
    def test_sh_output_is_one_shell_word_that_means_the_input(self) -> None:
        for text in self.texts(3):
            ...
            assert shlex.split(run("@sh", text), posix=True) == [text]
    ...
    def test_json_round_trip_of_random_data(self) -> None:
        ...
        for _ in range(200):
            ...
            for encode in ("to_json", "to_json(0)", "@json"):
                assert run(f".x | {encode} | from_json", data) == data["x"]
            assert json.loads(run(".x | to_json(0)", data)) == data["x"]
    ...
    def test_csv_round_trip_of_random_tables(self) -> None:
        ...
        for _ in range(80):
            ...
            assert run("@csv | @csvd", table) == rows
            assert run("@tsv | @tsvd", [[c.replace("\t", " ") for c in r] for r in table]) == rows
...
class SequenceProperties:
    def test_against_plain_python(self) -> None:
        ...
        for _ in range(150):
            ...
            assert run("reverse", xs) == xs[::-1]
            assert run("unique", xs) == list(dict.fromkeys(xs))
            assert run("filter(. > 4)", xs) == [x for x in xs if x > 4]
            assert run("sort", xs) == sorted(xs)
            assert run("[.[] as $x ireduce (0; . + $x)] | .[0]", xs) == sum(xs)
            assert run("contains([3])", xs) == (3 in xs)
            first = next((x for x in xs if x > 6), None)
            assert run("[first(. > 6)]", xs) == ([] if first is None else [first])

    def test_shuffle_is_a_permutation(self) -> None:
        rng = random.Random(9)
        for _ in range(60):
            xs = [rng.randint(0, 9) for _ in range(rng.randint(0, 12))]
            assert sorted(run("shuffle", xs)) == sorted(xs)
    ...
    def test_flatten_matches_a_recursive_flatten(self) -> None:
        ...
        for _ in range(120):
            ...
            assert run("flatten", data) == flatten(data, -1)
            for depth in (0, 1, 2):
                assert run(f"flatten({depth})", data) == flatten(data, depth)
    ...
    def test_group_by_and_unique_by_match_a_dict(self) -> None:
        ...
        for _ in range(100):
            ...
            assert run("group_by(.k)", rows) == list(groups.values())
            assert run("unique_by(.k)", rows) == [g[0] for g in groups.values()]
    ...
    def test_pick_omit_sort_keys_match_dict_operations(self) -> None:
        ...
        for _ in range(100):
            ...
            assert run(f"pick({json.dumps(chosen)})", data) == {k: data[k] for k in dict.fromkeys(chosen) if k in data}
            assert run(f"omit({json.dumps(chosen)})", data) == {k: v for k, v in data.items() if k not in chosen}
            assert list(run("sort_keys(.)", data)) == sorted(data)
    ...
    def test_pivot_is_a_transpose(self) -> None:
        ...
        for _ in range(60):
            ...
            assert run("pivot", matrix) == [list(col) for col in zip(*matrix)]
            assert run("pivot | pivot", matrix) == matrix
    ...
    def test_pivot_pads_ragged_rows_with_null(self) -> None:
        ...
        while checked < 60:
            ...
            assert run("pivot", rows) == expected
    ...
    def test_join_and_split_match_python(self) -> None:
        ...
        for _ in range(100):
            ...
            text = "|".join(items)
            assert run('join("|")', items) == text
            assert run('split("|")', text) == (text.split("|") if text else [])

    def test_case_operators(self) -> None:
        rng = random.Random(17)
        for _ in range(100):
            text = "".join(rng.choice("abcXYZ 019") for _ in range(rng.randint(0, 12)))
            assert run("upcase", text) == text.upper()
            assert run("downcase", text) == text.lower()
    ...
    def test_sub_and_match_agree_with_re_for_plain_patterns(self) -> None:
        ...
        for _ in range(150):
            ...
            assert run('sub("[ab]+", "<$0>")', text) == re.sub("[ab]+", lambda m: f"<{m.group(0)}>", text)
            assert run('sub("(a)(b)?", "$1-$2")', text) == re.sub("(a)(b)?", lambda m: f"{m.group(1)}-{m.group(2) or ''}", text)
            assert run('[match("[0-9]"; "g") | .offset]', text) == [m.start() for m in re.finditer("[0-9]", text)]
