...
class FormatTests:
    def test_go_constants(self) -> None:
        for name, (layout, expected) in GO_CONSTANTS.items():
            assert format_datetime(DOC_TIME, layout) == expected

    def test_the_custom_layout_from_the_yq_docs(self) -> None:
        moment = dt.datetime(2001, 12, 15, 2, 59, 43, 100000, tzinfo=UTC)
        assert format_datetime(moment, "Monday, 02-Jan-06 at 3:04PM") == "Saturday, 15-Dec-01 at 2:59AM"
        assert format_datetime(moment, "Monday") == "Saturday"
    ...
    def test_twelve_hour_clock(self) -> None:
        for hour, text in ((0, "12:00AM"), (1, "1:00AM"), (12, "12:00PM"), (13, "1:00PM")):
            ...
            assert format_datetime(moment, "3:04PM") == text
        assert format_datetime(dt.datetime(2020, 1, 1, 13, 5, tzinfo=UTC), "03:04pm") == "01:05pm"

    def test_offsets(self) -> None:
        tokyo = dt.timezone(dt.timedelta(hours=9), "JST")
        moment = dt.datetime(2020, 1, 2, 3, 4, 5, tzinfo=tokyo)
        assert format_datetime(moment, "-0700") == "+0900"
        assert format_datetime(moment, "-07:00") == "+09:00"
        assert format_datetime(moment, "-07") == "+09"
        assert format_datetime(moment, "-070000") == "+090000"
        assert format_datetime(moment, "-07:00:00") == "+09:00:00"
        assert format_datetime(moment, "Z07:00") == "+09:00"
        assert format_datetime(moment, "MST") == "JST"
        assert format_datetime(moment.astimezone(UTC), "Z07:00") == "Z"
        assert format_datetime(moment, RFC3339) == "2020-01-02T03:04:05+09:00"
    ...
    def test_an_unnamed_zone_is_written_as_its_offset(self) -> None:
        ...
        assert format_datetime(moment, "MST") == "-0530"

    def test_fractions(self) -> None:
        moment = dt.datetime(2020, 1, 2, 3, 4, 5, 120000, tzinfo=UTC)
        assert format_datetime(moment, "05.000") == "05.120"
        assert format_datetime(moment, "05.000000") == "05.120000"
        assert format_datetime(moment, "05.999") == "05.12"
        assert format_datetime(dt.datetime(2020, 1, 2, tzinfo=UTC), "05.999") == "00"
        assert format_datetime(moment, "05,000") == "05,120"

    def test_day_and_year_forms(self) -> None:
        moment = dt.datetime(2021, 3, 4, tzinfo=UTC)
        assert format_datetime(moment, "2 _2 02 1 01 Jan January 06 2006") == "4  4 04 3 03 Mar March 21 2021"
        assert format_datetime(moment, "002 __2") == "063  63"

    def test_literal_text_is_kept(self) -> None:
        moment = dt.datetime(2021, 3, 4, tzinfo=UTC)
        assert format_datetime(moment, "Year: 2006!") == "Year: 2021!"
        # "Janet" is not a month: the J is followed by a lowercase letter
        assert format_datetime(moment, "Janet") == "Janet"
...
class ParseTests:
    def test_go_constants_round_trip(self) -> None:
        for name, (layout, text) in GO_CONSTANTS.items():
            ...
            assert parsed == DOC_TIME
            assert format_datetime(parsed, layout) == text
    ...
    def test_the_layout_from_the_yq_docs(self) -> None:
        ...
        assert parsed == dt.datetime(2001, 12, 15, 2, 59, tzinfo=UTC)
        assert parsed.tzname() == "GMT"
        assert format_datetime(parsed, layout) == "Saturday, 15-Dec-01 at 2:59AM GMT"

    def test_unknown_abbreviations_keep_their_name_and_offset_zero(self) -> None:
        parsed = parse_datetime("15:04 MST", "10:30 AEDT")
        assert (parsed.utcoffset(), parsed.tzname()) == (dt.timedelta(0), "AEDT")
    ...
    def test_numeric_zones(self) -> None:
        for layout, text in (("15:04 -0700", "10:30 +0530"), ("15:04 -07:00", "10:30 +05:30"),
            ...
            assert parse_datetime(layout, text).utcoffset() == dt.timedelta(hours=5, minutes=30)
        assert parse_datetime("15:04 -07", "10:30 -08").utcoffset() == dt.timedelta(hours=-8)
        assert parse_datetime("15:04 Z0700", "10:30 Z").utcoffset() == dt.timedelta(0)
    ...
    def test_rfc3339_accepts_a_fraction_the_layout_does_not_mention(self) -> None:
        ...
        assert parsed == dt.datetime(2001, 12, 15, 2, 59, 43, 100000, tzinfo=UTC)

    def test_rfc3339_falls_back_to_a_date(self) -> None:
        assert parse_datetime(RFC3339, "2001-12-15") == dt.datetime(2001, 12, 15, tzinfo=UTC)

    def test_twelve_hour_clock(self) -> None:
        assert parse_datetime("3:04PM", "12:15AM").hour == 0
        assert parse_datetime("3:04PM", "12:15PM").hour == 12
        assert parse_datetime("3:04PM", "1:15PM").hour == 13
        assert parse_datetime("3:04pm", "1:15pm").hour == 13

    def test_two_digit_years(self) -> None:
        assert parse_datetime("06-01-02", "68-01-02").year == 2068
        assert parse_datetime("06-01-02", "69-01-02").year == 1969

    def test_month_names_are_case_insensitive(self) -> None:
        assert parse_datetime("02-Jan-2006", "23-dec-2010").month == 12
        assert parse_datetime("January 2, 2006", "MARCH 4, 2021").month == 3

    def test_spaces_and_padding(self) -> None:
        assert parse_datetime("Jan _2 2006", "Mar  4 2021").day == 4
        assert parse_datetime("2006-1-2", "2021-3-4").day == 4

    def test_day_of_year(self) -> None:
        assert parse_datetime("2006 002", "2021 063") == dt.datetime(2021, 3, 4, tzinfo=UTC)

    def test_fractions(self) -> None:
        assert parse_datetime("05.000", "07.120").microsecond == 120000
        assert parse_datetime("05.999", "07.12").microsecond == 120000
        assert parse_datetime("05.999", "07").microsecond == 0
        assert parse_datetime("05", "07.5").microsecond == 500000
    ...
    def test_rfc3339_error_is_from_the_first_layout(self) -> None:
        ...
        assert 'as "2006-01-02T15:04:05Z07:00"' in str(raised.value)


class UnixAndDurationTests:
    def test_unix_seconds_round_down(self) -> None:
        assert unix_seconds(dt.datetime(1970, 1, 1, 0, 0, 1, 999999, tzinfo=UTC)) == 1
        assert unix_seconds(dt.datetime(1969, 12, 31, 23, 59, 59, 500000, tzinfo=UTC)) == -1
        tokyo = dt.timezone(dt.timedelta(hours=9))
        assert unix_seconds(dt.datetime(2023, 2, 2, 10, 38, 49, tzinfo=tokyo)) == 1675301929

    def test_durations(self) -> None:
        assert parse_go_duration("3h1m") == dt.timedelta(hours=3, minutes=1)
        assert parse_go_duration("-1.5h") == dt.timedelta(minutes=-90)
        assert parse_go_duration("300ms") == dt.timedelta(milliseconds=300)
